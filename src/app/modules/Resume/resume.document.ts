...
const esc = (value: unknown): string => {
  ...
  return String(value).replace(/[&<>"']/g, (char) => ESCAPE_MAP[char] as string);
};
 ...
 */
const prettyUrl = (safe: string): string =>
  safe.replace(/^https?:\/\//i, '').replace(/^www\./i, '').replace(/\/$/, '');

const joinFilled = (parts: unknown[], separator = ' • '): string =>
  parts
    .map((p) => String(p ?? '').trim())
    .filter(Boolean)
    .map(esc)
 ...
 */
const formatDatePart = (value: unknown): string => {
  const raw = String(value ?? '').trim();
  if (!raw) return '';
  ...
  return month ? `${month} ${match[1]}` : (match[1] as string);
};

const dateRange = (start?: unknown, end?: unknown, current?: boolean): string => {
...
const renderSummary = (cv: IResumeContent): string =>
  section('Professional Summary', cv.summary ? `<p>${esc(cv.summary)}</p>` : '');

const renderExperience = (cv: IResumeContent): string =>
  section(
    'Work Experience',
    (cv.workExperience ?? [])
      .map(
...
const renderEducation = (cv: IResumeContent): string =>
  section(
    'Education',
    (cv.education ?? [])
      .map(
 ...
 */
const renderProjects = (cv: IResumeContent): string =>
  section(
    'Projects',
    (cv.projects ?? [])
      .map((project) => {
...
const renderSkills = (cv: IResumeContent): string =>
  section(
    'Skills',
    (cv.skills ?? []).length
      ? `<ul class="chips">${(cv.skills ?? [])
          .map(
            (skill) =>
              `<li>${esc(skill.name)}${skill.level ? `<span class="lvl">${esc(skill.level)}</span>` : ''}</li>`
          )
          .join('')}</ul>`
      : ''
  );

const renderCertifications = (cv: IResumeContent): string =>
  section(
    'Certifications',
    (cv.certifications ?? [])
      .map((cert) => {
...
const renderLanguages = (cv: IResumeContent): string =>
  section(
    'Languages',
    (cv.languages ?? []).length
      ? `<ul class="chips">${(cv.languages ?? [])
          .map(
            (lang) =>
              `<li>${esc(lang.name)}<span class="lvl">${esc(lang.proficiency)}</span></li>`
          )
          .join('')}</ul>`
      : ''
  );

const renderAwards = (cv: IResumeContent): string =>
  section(
    'Awards & Achievements',
    (cv.awards ?? [])
      .map(
...
const renderReferences = (cv: IResumeContent): string =>
  section(
    'References',
    (cv.references ?? [])
      .map(
 ...
 */
const renderContactLine = (cv: IResumeContent): string => {
  const parts: string[] = [];
...
const renderHeader = (
  cv: IResumeContent,
  opts: { bleed: boolean; stamp?: string }
): string => {
  const links = [
...
const RENDERERS: Record<SectionKey, (cv: IResumeContent) => string> = {
...
export const RESUME_SECTION_KEYS: SectionKey[] = DEFAULT_ORDER;
...
export const renderResumeDocument = (
  cv: IResumeContent,
  options: IRenderResumeOptions = {}
): string => {
  const templateId = isKnownTemplate(options.template)
    ? options.template
    : DEFAULT_RESUME_TEMPLATE_ID;
  ...
  // unexpected position.
  const order =
    templateId === 'custom' && options.sectionOrder?.length
      ? (() => {
          const chosen = options.sectionOrder.filter(
            (k): k is SectionKey => k in RENDERERS
          );
...
export const resumeFileName = (cv: IResumeContent, template?: string): string => {
  const name = String(cv.fullName || 'Candidate')
    .trim()
    .replace(/[^\p{L}\p{N}]+/gu, '_')
    .replace(/^_+|_+$/g, '');

  return `${name || 'Candidate'}_CV_${getTemplate(template).name.replace(/\s+/g, '')}`;
};
