...
const readFilters = (req: Request): ExportFilters => ({
 ...
 * performance win.
 */
const sendWorkbook = (res: Response, buffer: Buffer, baseName: string): void => {
...
const exportSheet = catchAsync(async (req: Request, res: Response) => {
  const { sheet } = req.params;

  if (!isSheetKey(sheet)) {
    throw new AppError(
      httpStatus.BAD_REQUEST,
      `Unknown report "${sheet}". Valid reports: ${SHEET_KEYS.join(', ')}`
    );
  }

  const spec = await SHEET_BUILDERS[sheet](readFilters(req));
  const buffer = await buildSingleSheetWorkbook(spec);

  sendWorkbook(res, buffer, SHEET_FILENAMES[sheet]);
});
 ...
 */
const exportFullWorkbook = catchAsync(async (req: Request, res: Response) => {
  const filters = readFilters(req);
  const workbook = createWorkbook();

  for (const key of SHEET_KEYS) {
    const spec = await SHEET_BUILDERS[key](filters);
    buildSheet(workbook, spec);
  }
...
const REPORT_CATALOGUE = [
  {
    ...
    description:
      'Every registered candidate with their CADD CORE student ID, batch, course, CV and portfolio links, job preferences and placement status.',
  ...
  },
  {
    ...
    description:
      'Registered companies with contact person, industry, location, website, the roles they are currently hiring for and their approval status.',
    supportsDateRange: true,
  ...
  },
  {
    ...
    description:
      'All job posts with salary range, required skills, experience, deadline, work mode and vacancy count.',
    supportsDateRange: true,
  ...
  },
  {
    ...
    description:
      'One row per application: who applied where, current status, whether a CV and cover letter went with it, and the follow-up state.',
    supportsDateRange: true,
  ...
  },
  {
    ...
    description:
      'One row per interview round, not per candidate — a candidate who sat three rounds appears three times, with venue, interviewers, rating and feedback.',
    supportsDateRange: true,
  ...
  },
  {
    ...
    description:
      'Confirmed hires with joining date, salary, placement source, employment status and the 6-month follow-up result.',
    supportsDateRange: true,
  ...
  },
  {
    ...
    description:
      "The placement cell's call log: who was contacted, when, why, what they said, their hiring need and the next action due.",
    supportsDateRange: true,
  ...
  },
  {
    ...
    title: 'Monthly KPI',
    description:
      'Month-by-month totals for students, employers contacted, vacancies, applications, interviews, selections, placements and placement rate, with a TOTAL row.',
    supportsDateRange: true,
  ...
  },
  {
    ...
    description:
      'A live snapshot of the placement funnel as Metric / Value / meaning rows, including shortlist, selection and rejection rates.',
    supportsDateRange: false,
    dateRangeMeaning: 'Always a live snapshot — date range does not apply.',
  },
];

const getReportCatalogue = catchAsync(async (_req: Request, res: Response) => {
  sendResponse(res, {
