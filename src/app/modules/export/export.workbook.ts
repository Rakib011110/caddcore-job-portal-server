...
const safeSheetName = (name: string): string =>
  name.replace(/[:\\/?*[\]]/g, '-').slice(0, MAX_SHEET_NAME);
...
const autoFitColumn = (
  ...
  for (const row of rows) {
    ...
    const length =
      value instanceof Date ? 10 : String(value).length;
    ...
    if (widest >= MAX_COL_WIDTH) return MAX_COL_WIDTH;
  }
...
export const buildSingleSheetWorkbook = async (
  spec: SheetSpec
): Promise<Buffer> => {
 ...
 */
export const xlsxDate = (value?: Date | string | null): Date | null => {
...
export const xlsxBool = (value?: boolean | null): string =>
  value === undefined || value === null ? '' : value ? 'Yes' : 'No';

/** Collapse a list into one cell. */
export const xlsxList = (
  value?: Array<string | number | null | undefined> | null
): string => (value?.length ? value.filter(Boolean).join('; ') : '');

/** Strip HTML and collapse whitespace, for description columns. */
export const xlsxText = (value?: string | null, maxLength = 500): string => {
  if (!value) return '';
